...
function validatePhotos(
  ...
  if (photos.length === 0) {
    return "Upload at least one clear photo of the dog.";
  }
...
export async function submitMissingDogReport(
  ...
  if (
    ...
    return {
      success: false,
      message:
        "You must be signed in before submitting a missing-dog report.",
    };
  }
  ...
  try {
    ...
    if (!dogId) {
      throw new Error(
        "Dog record was created without an ID.",
      );
    }
    ...
    for (
      ...
      const path =
        `${user.id}/${dogId}/${crypto.randomUUID()}.${safeExtension(
          photo,
        )}`;
