...
export const missingReportSchema = z
  .object({
    ...
    lastSeenAt: z
      ...
      .min(
        1,
        "Last-seen date and time are required.",
      )
      .refine(
        (value) =>
          !Number.isNaN(
    ...
    (data, ctx) => {
      if (
        ...
        ctx.addIssue({
          ...
          path: ["rewardAmount"],
          message:
            "Enter the reward amount or turn off the reward option.",
        });
      }
      ...
      if (
        ...
        ctx.addIssue({
          ...
          path: ["rewardAmount"],
          message:
            "Turn on the reward option before entering an amount.",
        });
      }
    },
  );

export type MissingReportInput =
  z.infer<
    typeof missingReportSchema
  >;

export function missingReportFromFormData(
  formData: FormData,
) {
  return {
    dogName: String(
    ...
    microchipped:
      formData.get("microchipped") === "on",

    lastSeenAt: String(
      formData.get("lastSeenAt") ?? "",
    ),

    timeIsApproximate:
      formData.get("timeIsApproximate") === "on",
